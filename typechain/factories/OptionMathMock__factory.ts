/* Autogenerated file. Do not edit manually. */
/* tslint:disable */
/* eslint-disable */

import { Contract, Interface, type ContractRunner } from "ethers";
import type {
  OptionMathMock,
  OptionMathMockInterface,
} from "../OptionMathMock";

const _abi = [
  {
    inputs: [],
    name: "OptionMath__NonPositiveVol",
    type: "error",
  },
  {
    inputs: [
      {
        internalType: "UD60x18",
        name: "min",
        type: "uint256",
      },
      {
        internalType: "UD60x18",
        name: "max",
        type: "uint256",
      },
      {
        internalType: "UD60x18",
        name: "price",
        type: "uint256",
      },
    ],
    name: "OptionMath__OutOfBoundsPrice",
    type: "error",
  },
  {
    inputs: [],
    name: "OptionMath__Underflow",
    type: "error",
  },
  {
    inputs: [
      {
        internalType: "uint256",
        name: "x",
        type: "uint256",
      },
      {
        internalType: "uint256",
        name: "y",
        type: "uint256",
      },
    ],
    name: "PRBMath_MulDiv18_Overflow",
    type: "error",
  },
  {
    inputs: [
      {
        internalType: "uint256",
        name: "x",
        type: "uint256",
      },
      {
        internalType: "uint256",
        name: "y",
        type: "uint256",
      },
      {
        internalType: "uint256",
        name: "denominator",
        type: "uint256",
      },
    ],
    name: "PRBMath_MulDiv_Overflow",
    type: "error",
  },
  {
    inputs: [],
    name: "PRBMath_SD59x18_Abs_MinSD59x18",
    type: "error",
  },
  {
    inputs: [],
    name: "PRBMath_SD59x18_Div_InputTooSmall",
    type: "error",
  },
  {
    inputs: [
      {
        internalType: "SD59x18",
        name: "x",
        type: "int256",
      },
      {
        internalType: "SD59x18",
        name: "y",
        type: "int256",
      },
    ],
    name: "PRBMath_SD59x18_Div_Overflow",
    type: "error",
  },
  {
    inputs: [
      {
        internalType: "SD59x18",
        name: "x",
        type: "int256",
      },
    ],
    name: "PRBMath_SD59x18_Exp2_InputTooBig",
    type: "error",
  },
  {
    inputs: [
      {
        internalType: "SD59x18",
        name: "x",
        type: "int256",
      },
    ],
    name: "PRBMath_SD59x18_Exp_InputTooBig",
    type: "error",
  },
  {
    inputs: [
      {
        internalType: "SD59x18",
        name: "x",
        type: "int256",
      },
    ],
    name: "PRBMath_SD59x18_IntoUD60x18_Underflow",
    type: "error",
  },
  {
    inputs: [
      {
        internalType: "SD59x18",
        name: "x",
        type: "int256",
      },
    ],
    name: "PRBMath_SD59x18_Log_InputTooSmall",
    type: "error",
  },
  {
    inputs: [],
    name: "PRBMath_SD59x18_Mul_InputTooSmall",
    type: "error",
  },
  {
    inputs: [
      {
        internalType: "SD59x18",
        name: "x",
        type: "int256",
      },
      {
        internalType: "SD59x18",
        name: "y",
        type: "int256",
      },
    ],
    name: "PRBMath_SD59x18_Mul_Overflow",
    type: "error",
  },
  {
    inputs: [
      {
        internalType: "UD60x18",
        name: "x",
        type: "uint256",
      },
    ],
    name: "PRBMath_UD60x18_IntoSD59x18_Overflow",
    type: "error",
  },
  {
    inputs: [
      {
        internalType: "UD60x18",
        name: "x",
        type: "uint256",
      },
    ],
    name: "PRBMath_UD60x18_Sqrt_Overflow",
    type: "error",
  },
  {
    inputs: [
      {
        internalType: "UD60x18",
        name: "spot",
        type: "uint256",
      },
      {
        internalType: "UD60x18",
        name: "strike",
        type: "uint256",
      },
      {
        internalType: "UD60x18",
        name: "timeToMaturity",
        type: "uint256",
      },
      {
        internalType: "UD60x18",
        name: "volAnnualized",
        type: "uint256",
      },
      {
        internalType: "UD60x18",
        name: "riskFreeRate",
        type: "uint256",
      },
      {
        internalType: "bool",
        name: "isCall",
        type: "bool",
      },
    ],
    name: "blackScholesPrice",
    outputs: [
      {
        internalType: "UD60x18",
        name: "",
        type: "uint256",
      },
    ],
    stateMutability: "pure",
    type: "function",
  },
  {
    inputs: [
      {
        internalType: "UD60x18",
        name: "strike",
        type: "uint256",
      },
    ],
    name: "calculateStrikeInterval",
    outputs: [
      {
        internalType: "UD60x18",
        name: "",
        type: "uint256",
      },
    ],
    stateMutability: "pure",
    type: "function",
  },
  {
    inputs: [
      {
        internalType: "uint256",
        name: "maturity",
        type: "uint256",
      },
    ],
    name: "calculateTimeToMaturity",
    outputs: [
      {
        internalType: "uint256",
        name: "",
        type: "uint256",
      },
    ],
    stateMutability: "view",
    type: "function",
  },
  {
    inputs: [
      {
        internalType: "UD60x18",
        name: "spot",
        type: "uint256",
      },
      {
        internalType: "UD60x18",
        name: "strike",
        type: "uint256",
      },
      {
        internalType: "UD60x18",
        name: "timeToMaturity",
        type: "uint256",
      },
      {
        internalType: "UD60x18",
        name: "volAnnualized",
        type: "uint256",
      },
      {
        internalType: "UD60x18",
        name: "riskFreeRate",
        type: "uint256",
      },
    ],
    name: "d1d2",
    outputs: [
      {
        internalType: "SD59x18",
        name: "d1",
        type: "int256",
      },
      {
        internalType: "SD59x18",
        name: "d2",
        type: "int256",
      },
    ],
    stateMutability: "pure",
    type: "function",
  },
  {
    inputs: [
      {
        internalType: "SD59x18",
        name: "x",
        type: "int256",
      },
    ],
    name: "helperNormal",
    outputs: [
      {
        internalType: "SD59x18",
        name: "",
        type: "int256",
      },
    ],
    stateMutability: "pure",
    type: "function",
  },
  {
    inputs: [
      {
        internalType: "UD60x18",
        name: "spot",
        type: "uint256",
      },
      {
        internalType: "UD60x18",
        name: "strike",
        type: "uint256",
      },
      {
        internalType: "uint256",
        name: "maturity",
        type: "uint256",
      },
    ],
    name: "initializationFee",
    outputs: [
      {
        internalType: "UD60x18",
        name: "",
        type: "uint256",
      },
    ],
    stateMutability: "view",
    type: "function",
  },
  {
    inputs: [
      {
        internalType: "uint256",
        name: "maturity",
        type: "uint256",
      },
    ],
    name: "isFriday",
    outputs: [
      {
        internalType: "bool",
        name: "",
        type: "bool",
      },
    ],
    stateMutability: "pure",
    type: "function",
  },
  {
    inputs: [
      {
        internalType: "uint256",
        name: "maturity",
        type: "uint256",
      },
    ],
    name: "isLastFriday",
    outputs: [
      {
        internalType: "bool",
        name: "",
        type: "bool",
      },
    ],
    stateMutability: "pure",
    type: "function",
  },
  {
    inputs: [
      {
        internalType: "UD60x18",
        name: "spot",
        type: "uint256",
      },
      {
        internalType: "UD60x18",
        name: "strike",
        type: "uint256",
      },
    ],
    name: "logMoneyness",
    outputs: [
      {
        internalType: "UD60x18",
        name: "",
        type: "uint256",
      },
    ],
    stateMutability: "pure",
    type: "function",
  },
  {
    inputs: [
      {
        internalType: "SD59x18",
        name: "x",
        type: "int256",
      },
    ],
    name: "normalCdf",
    outputs: [
      {
        internalType: "SD59x18",
        name: "",
        type: "int256",
      },
    ],
    stateMutability: "pure",
    type: "function",
  },
  {
    inputs: [
      {
        internalType: "SD59x18",
        name: "x",
        type: "int256",
      },
    ],
    name: "normalPdf",
    outputs: [
      {
        internalType: "SD59x18",
        name: "",
        type: "int256",
      },
    ],
    stateMutability: "pure",
    type: "function",
  },
  {
    inputs: [
      {
        internalType: "UD60x18",
        name: "spot",
        type: "uint256",
      },
      {
        internalType: "UD60x18",
        name: "strike",
        type: "uint256",
      },
      {
        internalType: "UD60x18",
        name: "timeToMaturity",
        type: "uint256",
      },
      {
        internalType: "UD60x18",
        name: "volAnnualized",
        type: "uint256",
      },
      {
        internalType: "UD60x18",
        name: "riskFreeRate",
        type: "uint256",
      },
      {
        internalType: "bool",
        name: "isCall",
        type: "bool",
      },
    ],
    name: "optionDelta",
    outputs: [
      {
        internalType: "SD59x18",
        name: "",
        type: "int256",
      },
    ],
    stateMutability: "pure",
    type: "function",
  },
  {
    inputs: [
      {
        internalType: "SD59x18",
        name: "x",
        type: "int256",
      },
    ],
    name: "relu",
    outputs: [
      {
        internalType: "UD60x18",
        name: "",
        type: "uint256",
      },
    ],
    stateMutability: "pure",
    type: "function",
  },
] as const;

export class OptionMathMock__factory {
  static readonly abi = _abi;
  static createInterface(): OptionMathMockInterface {
    return new Interface(_abi) as OptionMathMockInterface;
  }
  static connect(
    address: string,
    runner?: ContractRunner | null
  ): OptionMathMock {
    return new Contract(address, _abi, runner) as unknown as OptionMathMock;
  }
}
